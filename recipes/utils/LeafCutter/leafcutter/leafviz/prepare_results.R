...
exons_table=if (!is.null( exon_file )) {
  ...
  as.data.frame(fread(file = exon_file), data.table=F )
} else {
...
intron_db <- fread(file = all_introns, data.table = FALSE)
...
threeprime_db <- fread(file = threeprime_file, data.table = FALSE)
...
fiveprime_db <- fread(file = fiveprime_file, data.table = FALSE)
