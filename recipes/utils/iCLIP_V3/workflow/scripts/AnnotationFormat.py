...
def CombineTypes(row):
    ...
    for strand in ['Same', 'Oppo']:
        ...
        if row[strand+'_gene_type'] == '.' and row[strand+'_Repeat'] == '.' and str(row[strand+'_intron_number']) in ['.', ' ', '']:
...
def MergeDataframes(SameStrandGen_df, SameStrandIntrons_df, SameStrandRMSK_df, OppoStrandGen_df, OppoStrandIntrons_df, OppoStrandRMSK_df):
    ...
    Out_df = pd.DataFrame(columns = ['ID', 'chrom', 'start', 'end', 'score', 'strand', 'Length', 'Counts_Unique', 'Counts_fracMM', 'Counts_Total',\
                                 ...
                                 'Same_exon_Overlap',      'Same_Repeat',                'Same_Comb_type',\
                                 ...
                                 'Oppo_exon_Overlap',      'Oppo_Repeat',                'Oppo_Comb_type'])

    num_columns = len(Out_df.columns) - 1  # Subtract 1 to exclude the 'ID' column

    #print(SameStrandGen_df.dtypes)
    #print(SameStrandIntrons_df.dtypes)
    #print(SameStrandRMSK_df.dtypes)
    ...
    #print(Out_df[['ID','Same_type','Same_Repeat_LTR']])
...
def main():
    ...
    Out_df = MergeDataframes(SameStrandGen_df, SameStrandIntrons_df, SameStrandRMSK_df, OppoStrandGen_df, OppoStrandIntrons_df, OppoStrandRMSK_df)
