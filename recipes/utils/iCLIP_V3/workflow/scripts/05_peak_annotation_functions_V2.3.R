...
##########################  
  ...
  print("merge bedfiles into output text")
...
##########################  
  ...
  print("subset for annotations with annotations")
  ...
  if (nrow(ab_OL)>0){
    ...
    ####################################
    print("Process table of all annotations for regions with multiple annotations")

    #ab_OL_double <- head(ab_OL_double, n=20)
    ...
    rownames(ab_OL_colapsed) <- unique_list    
    ab_OL_colapsed$ID <- unique_list
    ab_OL_colapsed$ID.1 <- unique_list

    #for every unique id with multiple annotations

    #print('START FOR LOOP')

    #print(ab_OL_double[which(ab_OL_double$ID == "chr1:133659752-133659832_-"),])

    #print(head(ab_OL_colapsed))


    update_ab_OL_colapsed <- function(ab_OL_colapsed){

      paste_unique <- function(x) {
        return(paste0(unique(x), collapse = ','))
      }

      ID = ab_OL_colapsed['ID']
      sub_ab_OL_double <- ab_OL_double[which(ab_OL_double$ID == ID),]
      
      ab_OL_colapsed <- apply(sub_ab_OL_double, 2, paste_unique) 
      #ab_OL_colapsed['chr'] = paste0(unique(sub_ab_OL_double$chr), collapse = ',')
      #ab_OL_colapsed['start'] = paste0(unique(sub_ab_OL_double$start), collapse = ',')
      #ab_OL_colapsed['end'] = paste0(unique(sub_ab_OL_double$end), collapse = ',')
      #ab_OL_colapsed['strand'] = paste0(unique(sub_ab_OL_double$strand), collapse = ',')
      #ab_OL_colapsed['chr_anno'] = paste0(unique(sub_ab_OL_double$chr_anno), collapse = ',')
      #ab_OL_colapsed['start_anno'] = paste0(unique(sub_ab_OL_double$start_anno), collapse = ',')
      #ab_OL_colapsed['end_anno'] = paste0(unique(sub_ab_OL_double$end_anno), collapse = ',')
      #ab_OL_colapsed['strand_anno'] = paste0(unique(sub_ab_OL_double$strand_anno), collapse = ',')
      #ab_OL_colapsed['ensembl_gene_id'] = paste0(unique(sub_ab_OL_double$ensembl_gene_id), collapse = ',')
      #ab_OL_colapsed['external_gene_name'] = paste0(unique(sub_ab_OL_double$external_gene_name), collapse = ',')
      #ab_OL_colapsed['gene_type'] = paste0(unique(sub_ab_OL_double$gene_type), collapse = ',')
      #ab_OL_colapsed['gene_type_ALL'] = paste0(unique(sub_ab_OL_double$gene_type_ALL), collapse = ',')
      #ab_OL_colapsed['ntOL'] = paste0(unique(sub_ab_OL_double$ntOL), collapse = ',')
      #ab_OL_colapsed['width'] = paste0(unique(sub_ab_OL_double$width), collapse = ',')
      #ab_OL_colapsed['width_anno'] = paste0(unique(sub_ab_OL_double$width_anno), collapse = ',')
      #ab_OL_colapsed['OLper_anno'] = paste0(unique(sub_ab_OL_double$OLper_anno), collapse = ',')
      #ab_OL_colapsed['OLper'] = paste0(unique(sub_ab_OL_double$OLper), collapse = ',')

      #if (ID == 'chr1:135430357-135430469_-') { print(sub_ab_OL_double)}
      #if (ID == 'chr1:135430357-135430469_-') { print(ab_OL_colapsed)}
    ...
    ab_OL_colapsed <- t(apply(ab_OL_colapsed, 1, update_ab_OL_colapsed))

    print(head(ab_OL_colapsed))

    #print('test')
    #print(nrow(ab_OL_colapsed))
    #print('END FOR LOOP')
    
    #print(head(ab_OL_colapsed))
    #print(ab_OL_colapsed[which(ab_OL_colapsed$ID == "chr1:135430357-135430469_-"),])
    #print(ab_OL_colapsed["chr1:135430357-135430469_-",])
    #print(ab_OL_single[which(ab_OL_single$ID == "chr1:135430357-135430469_-"),])
...
peak_calling<-function(peak_in,nmeprfix){
  ...
  # change all pseudogene classes to pseudogene
  ...
  system.time({ 
  if(length(anno)>0) { 
    UniqueIDs=unique(peaksTable_Anno$ID)

    peaksTable_colapsed=as.data.frame(matrix(nrow=length(UniqueIDs),ncol=ncol(peaksTable_Anno)));
    colnames(peaksTable_colapsed) <- colnames(peaksTable_Anno)
    rownames(peaksTable_colapsed) <- UniqueIDs
    peaksTable_colapsed$ID <- UniqueIDs
    ...
    print(paste0('Annotated Regions: ', length(UniqueIDs)))

    update_peaksTable_colapsed <- function(peaksTable_colapsed) {
      ID = peaksTable_colapsed['ID']
      pam <- peaksTable_Anno[which(peaksTable_Anno$ID == ID),]
      ################
      # Comb Annotation
      ...
      peaksTable_colapsed[colnames(peaksTable_Anno)]=as.character(pam[1,colnames(peaksTable_Anno),drop=T])
      peaksTable_colapsed['type_simple_comb']=pam_c2
      peaksTable_colapsed['type_comb']=pam_c
      peaksTable_colapsed['gene_name_comb']=pam_gmerge

      return(peaksTable_colapsed)
    }

    peaksTable_colapsed <- t(apply(peaksTable_colapsed, 1, update_peaksTable_colapsed))    
...
IntronExon_prep=function(gencode_path,intron_path,gencode_transc_path){
  ...
  #ref_gencode_e = ref_gencode[which(ref_gencode$feature == "exon"),]
  #ref_gencode_e = ref_gencode %>% filter(feature == "exon")
  #print(head(ref_gencode))
  #introns
...
#print(head(anno_IntExn))  
  ...
  print('setting up bedtools intersect')
