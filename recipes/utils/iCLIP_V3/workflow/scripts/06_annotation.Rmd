...
peak_heigth=as.numeric(params$peak_heigth)
...
Identify peaks with All non-spliced reads (unique and Multimapping) and count number of reads that overlap with the identified peaks. Peaks with minimum height >= `r toString(peak_heigth)` were assessed.  
...
<!-- **ncRNA > Protein coding : Exonic > repeats > Pseudogene > CHANGED: Antisense Feature > Protein Coding : Intronic > lncRNA > no Feature**    -->

**ncRNA > protein_coding: Exonic > Repeat Element > pseudogene > lncRNA: Exonic > protein_coding: Intronic > lncRNA: Intronic > no Feature**
...
p$RNA=''
#plot
ggplot(p,aes(fill=RNA_Type,x=RNA_Type)) +
  geom_bar(stat="count") +
  theme_classic() +
