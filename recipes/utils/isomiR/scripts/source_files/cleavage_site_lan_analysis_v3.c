...
// July-27-2018 v3
// Due to the fact that antisense (-strand) sequence is different from sense (+strand), maybe we should not simply use the ends of reads mapping to antisense to indicate cleavage sites on sense,
// instead, in this version, I plan to use the ends of reads mapping to antisense (-strand) to infer cleavage site sequence of antisense.
// The cleavage site inferred from antisense will count differently from those of sense sequences.
// program changes are: 1) analysis mode 2)reference_sta 3)outputs

// analysis_mode:               1: Both 5' and 3' ends count to support a cleavage site
//                              2: only 5' of read counts
...
int         reference_sta[MAX_REF_SIZE][6] = {0};         // 0: conditional sum sense ; 1: conditional sum antisense; 2: 5' end + ; 3: 3' end + ; 4: 5' end - ; 5: 3' end 1 ;

/*                                         FUNTION PROTOTYPE HERE                          */

int              print_cleavage_motif(int pos, int flag);
...
int main(int argc, char* argv[])		// Five arguments allowed
{
    ...
    if (argc != 6)
	{
        fprintf(stderr, "\n cleavage_site_lan_analysis_v3.exe reference input_SAM_file  anlysis_mode:1(both 5' end and 3' end) or 2(only 5' end counts)) threshold(posive number) output_file_option(on/off)\n");
		exit (1);
	}
    ...
    mode = atoi(argv[3]);
    if (mode != 1 && mode != 2)
    {fprintf(stderr, "analysis_mode should be either 1 or 2!\n"); exit (1);}        // error!
    ...
    // reference_sta[pos][] 0: conditional sum sense ; 1: conditional sum antisense; 2: 5' end + ; 3: 3' end + ; 4: 5' end - ; 5: 3' end 1 ;
    ...
    while (fgets(line, MAX_LINE_SIZE, fp_input_sam) != NULL)
    {
        ...
        {
            reference_sta[pos][2]++;                //5' end
            reference_sta[pos+length][3]++;         //3' end
        }
        else if (flag == 16)    // mapped to the -strand
        {
            reference_sta[pos+length][4]++;       //5' end
            reference_sta[pos][5]++;              //3' end
        }
    }
    
    for(i=1; i<reference_length;i++)
    {
        if (mode == 1) {reference_sta[i][0] = reference_sta[i][2] + reference_sta[i][3]; reference_sta[i][1] = reference_sta[i][4] + reference_sta[i][5];}
        else if (mode == 2) {reference_sta[i][0] = reference_sta[i][2]; reference_sta[i][1] = reference_sta[i][4];}
        else {fprintf(stderr, "Illigal mode number!\n"); exit (1);}
    }
    
    fprintf(stdout, "Pos\tStrand\tCount(+)\tCount(-)\t5+\t3+\t5-\t3-\n");
    for(i=1; i<reference_length;i++)
    {
        if (reference_sta[i][0] >= threshold  )
        {
            flag = 0;                // sense
            num_cleavage_site++;
            printf("%d\t%c\t%d\t%d\t%d\t%d\t%d\t%d\n",i, flag?'-':'+', reference_sta[i][0], reference_sta[i][1], reference_sta[i][2], reference_sta[i][3], reference_sta[i][4],reference_sta[i][4] );
            if (output_file_option == 1) {if (print_cleavage_motif(i, flag) == 1) skipped++;};
        }
        if (reference_sta[i][1] >= threshold )
        {
            flag = 1;               // antisense
            num_cleavage_site++;
            printf("%d\t%c\t%d\t%d\t%d\t%d\t%d\t%d\n",i, flag?'-':'+', reference_sta[i][0], reference_sta[i][1], reference_sta[i][2], reference_sta[i][3], reference_sta[i][4],reference_sta[i][4] );
...
int    print_cleavage_motif(int pos, int flag)                               // return 0 if sucessful, return 1 if too close to the ends!
{
