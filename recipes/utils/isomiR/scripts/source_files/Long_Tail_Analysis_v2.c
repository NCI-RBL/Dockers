...
// 2019 April 2nd afternoon V2

// ***************************************************************************************************************************************
// *    Long_Tail_Analysis_v2.exe input.tsv name_miRNA min_isomir_length max_isomir_length mode                                          *
// ***************************************************************************************************************************************

// The major change is how we filter the tailed reads, in v1, we focused on isomiRs with tails of certain lengths. in V2, we focused on isomiRs of certain length overall -> those isomirs appears on the upper part of NB gel!
// the mode is still useless
...
int                 min_isomir_length;
int                 max_isomir_length;
FILE                *fp_input;
...
int main(int argc, char* argv[])		// three arguments allowed
{
    ...
    min_isomir_length = atoi (argv[3]);
    max_isomir_length = atoi (argv[4]);
    ...
    while(fgets(line, MAX_char_per_line, fp_input) != NULL)
    {
        ...
        sscanf(line, "%s%*s%d%d%*s%d%d%s", NAME, &LEN_READ, &READS, &LEN_TRIM, &LEN_TAIL, SEQ_TAIL);
        ...
        if (LEN_READ < min_isomir_length || LEN_READ > max_isomir_length) continue;
        ...
        printf("num_total_tailed between %d and %d is %d\n",min_isomir_length, max_isomir_length, num_total_tailed_analyzed);
