/********************************************************************************************************************************************
 *                                                                                                                                          *
 * this grogram deal with raw data in the format of fastq, assuming the read is read1                                                       *
 *                                                                                                                                          *
 * program will look for the MOTIF1 which is CTGAC in this case. If sucessful, it will trim everything after it; otherwise, it will leave it*
 * program will also generate information regarding the sequencing processing, print to stderr                                              *
 *                                                                                                                                          *
 *             Commond --    trim_read1 < input.fastq > output.fastq                                                                        *
 *                                                                                                                                          *
...
#define	LEN_NAME	100              /* need to define the data structure for fastq read */
#define	LEN_SEQ		100
#define	LEN_TAG		16
...
Bool        trim_read(struct node_fastq *);
...
int main(int argc, char* argv[])		/* no arguments allowed */
{
	if (argc != 1) 
	{
		fprintf(stderr, "\n trim_read1 < input.fastq > output.fastq \n");
		return 0;
	}
	...
	{	
        ...
        if (trim_read(p_cur) == SUCCESS ) num_reads_trimmed++;
        
        print_fastq( &(p_cur->read));
	}

    fprintf(stderr, "\n\nTotal number of read is:      %ld\n", num_total_reads);
    fprintf(stderr, "Number of read trimmed is:    %ld, that is %ld%% of total reads\n", num_reads_trimmed, num_reads_trimmed*100/num_total_reads);
	return 0;	
}
...
Bool        trim_read(struct node_fastq * p_nod)
{
    char    *p_seq = p_nod->read.sequence;
	char	*p_motif1_start;
    
    if ((p_motif1_start = strstr(p_seq, MOTIF1)) == NULL ) return FAIL;         /* find the frist MOTIF1 CTGAC */
    else    *p_motif1_start = '\0';                                             /* truncate everything after MOTIF1 */
