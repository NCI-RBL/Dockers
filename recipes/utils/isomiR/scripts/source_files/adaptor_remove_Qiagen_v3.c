/********************************************************************************************************************************************
 *                                                                                                                                          *
 * this program deal with raw data in the format of fastq, assuming the read is read1 which contains 15nt randam sequences tag at the end   *
 * of 3'end                                                                                                                                 *
 * program reads fasta file of read1 from stdin. The final output is sequences ready to be mapped in fastq, which will be print to stdout  *
 * program will also generate information regarding the sequencing processing, print to stderr                                              *
 *                                                                                                                                          *
 *             Command --    analyze_read1 mismatch_allowed minimal_size_pass_filter < input.fastq > output.fastq                           *
 *                                                                                                                                          *
...
//  2014-Dec-18 : v2
//  find the first CTGAC-15N-TGGAA within mismatches
//  shortage for motif2 is allowed but counted in number of mismatches

//  2015-Jan-30 :   v3
//  changed the file name to trim_read1_miv_v3.c
//  this one changed the way we looking for adaptors -- instead of counting #mismatch, using minimal matches and pass_rate
...
//  2020-Aug-20: v1
//  make a big change: to analyze the library created by the Qiagen kit
//  the reads are xxxAACTGTAGGCACCATCAAT(19nt)-12N-AGATCGGAAGAGC

//  2020-Aug-21: v2
//  try to eliminate any low qulity reads (containing "N")

//  2020-Aug-21: v3
//  make it less strigent when comparing reads and tags -> we will capture more potential duplicated reads
...
#define ADAPTOR		"AACTGTAGGCACCATCAATNNNNNNNNNNNNAGATCGGAAGAGCACACGTCTGA"   // 19nt+12N+23nt = 42nt+12N = 54nt
...
#define THREE_ADAPTOR_MIN_MATCH         18           // in the first 19nt, there can be upto 1 mismatch

#define FIVE_ADAPTOR_PASS_RATE          0.9
#define FIVE_ADAPTOR_MIN_MATCH          20

#define TAG_PASS_RATE                   0.8         // allowing up to 2 mismatches in the 12N tag..
#define TAG_MIN_MATCH                   10

#define READ_PASS_RATE                  0.8         // allowing up to 2 mismatches in the frist 12nt
#define READ_MIN_MATCH                  10

#define FIVE_ADAPTOR    "GTTCAGAGTTCTACAGTCCGACGATC"    //5' adaptor seq new for Qiagen 26nt
...
int main(int argc, char* argv[])		/* One argument allowed */
{
	if (argc != 2)
	{
		fprintf(stderr, "\n adaptor_remove_Qiagen_v1.exe min_length_pass_filter < input.fastq > output.fastq \n");
		return 0;
	}
    ...
    int         num_total_reads = 0, num_contain_adaptor = 0, num_empty = 0, num_too_short = 0, num_5adapotr_contamination = 0, num_final = 0, num_duplicated = 0, num_low_quality = 0;
	...
	{	
        ...
        if (remove_adaptor(p_cur))
        {
            ...
            else if (strchr(p_cur->read.sequence, 'N') != NULL) num_low_quality++;
    ...
    fprintf(stderr, "Number of low quality reads :       %15d, that is %d%% of reads passing adaptor removal\n", num_low_quality, num_low_quality*100/num_contain_adaptor);
...
Bool	found_adaptors(char* read)    // modifie so not it works on the Qiagen system
{
                char*       key = ADAPTOR;
                int         count = 0;
...
Bool	remove_adaptor( struct node_fastq* p_nod)
{
        ...
        while ( *p )  // go over the whole sequence string
        {
            if ( found_adaptors(p) )
            {
                ...
                *(p + 31) = '\0';                               //mark the end of the 12N
                strcpy(p_nod->read.tag, p + 19);               // moved tag seq to third line
...
long int    key_value(char* p_key)              /* calculate key value for a key of KAY_SIZE nt tag sequences */
{
    long int    value = 0;
    int         i;
