...
//  2015-Jun-23 : v4_5nt
//  Modify the program to search and removal 3' adaptor with CTGAC+15N (old adaptor)
//  Constent changed: ADAPTOR, THREE_ADAPTOR_MIN_MATCH, THREE_ADAPTOR_PASS_RATE
//  Function changed: adaptor_search, adaptor_removal
...
#define ADAPTOR		"CTGACNNNNNNNNNNNNNNNTGGAATTCTCGGGT"

#define KAY_SIZE            7
#define HASH_TABLE_SIZE     16384        //key word size is 4^7


#define THREE_ADAPTOR_PASS_RATE         0.85        // so 6/7 = 0.857 will pass...
#define THREE_ADAPTOR_MIN_MATCH         5           // the CTGAC are match OR one mismatches need to be compensated by two matches after 15N
...
Bool	remove_adaptor( struct node_fastq* p_nod)
{
        ...
        while ( *p )  // go over the whole sequence string
        {
            if ( found_adaptors(p) )
            {
                ...
                *(p + 20) = '\0';                               //mark the end of the 15N
                strcpy(p_nod->read.tag, p + 5);               // moved tag seq to third line
