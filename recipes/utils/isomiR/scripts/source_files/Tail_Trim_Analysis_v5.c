...
// 2017 Aug 12 v4
// take UPPER_trim_SIZE and UPPER_tail_SIZE as limits, then take the actual upper_trim_size and upper_tail_size from inputs.
// ***************************************************************************************************************************************
// *    Tail_Trim_analysis-v4.exe input.tsv output.txt upper_trim_size upper_tail_size                                                   *
// ***************************************************************************************************************************************

// 2017 Aug 29 v5
// make change so it now recoginze the new input format of xavier's files....
// be aware of that the SEQ_TAIL is in the format of "AATTxx" (the "" are there!)
...
#define UPPER_trim_SIZE      10              // max length of Trimming
#define UPPER_tail_SIZE      40              // max length of tail sequence
...
int main(int argc, char* argv[])		// three arguments allowed
{
    ...
    upper_trim_size = atoi (argv[3]);
    upper_tail_size = atoi (argv[4]);
    
    if ( (upper_trim_size > UPPER_trim_SIZE) || (upper_tail_size > UPPER_tail_SIZE))
    {
        fprintf(stderr, "upper_trim_size or upper_tail_size is too large! (10 and 40)\n");
        exit (1);
    }
    ...
    while(fgets(line, MAX_char_per_line, fp_input) != NULL)
    {
        ...
        sscanf(line, "%*s%*s%*s%d%d%*s%d%d%s%d", &LEN_READ, &READS, &LEN_TRIM, &LEN_TAIL, SEQ_TAIL, &VAR_5P);
