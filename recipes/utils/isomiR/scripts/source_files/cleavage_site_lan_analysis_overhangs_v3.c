...
//Jan-9-2019
//The third version is based on v1 but not v2
//Here, we first identify a positive read with an abundance over the threshold (based on 5+ or [1]), then check if there is a negative read to match (3- or [4])
//Then we do the same for nagtive strand, read 5- [3], then check 3+ [2]
//So, only uniqu read counts!
//Report statistics.
...
int main(int argc, char* argv[])		// Five arguments allowed
{
    ...
    for(i=1; i<reference_length;i++)
    {
        ...
        {
            num_plus++;
            if (reference_sta[i][4] != 0) num_plus_yes++;               // this position is suported by both plus(5) and minus strand(3')
        }
    }
    
    for(i=1; i<reference_length;i++)
    {
        if (reference_sta[i][3] >= threshold)
        {
            num_minus++;
            if (reference_sta[i][2] != 0) num_minus_yes++;               // this position is suported by both plus(5) and minus strand(3')
        }
    }
