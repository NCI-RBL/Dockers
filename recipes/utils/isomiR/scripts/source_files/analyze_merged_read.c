/********************************************************************************************************************************************
 *                                                                                                                                          *
 * this grogram deal with merged reads in the format of fastq, assuming the read is in the direction of read1 which contains 15nt randam    *
 * sequences tag at the end                                                                                                                 *
 *                                                                                                                                          *
 * program reads fastq file of reads from stdin. The final output is sequnences ready to be mapped in fastq, which will be print to stdout  *
 * program will also generate information regarding the sequencing processing, print to stderr                                              *
 *                                                                                                                                          *
 *             Commond --    analyze_merged_read1 < input.fastq > output.fastq                                                              *
 *                                                                                                                                          *
 *              Constants defined are   1.  length of the read  -- 76                                                                       *
 *                                      2.  length of tag       -- 16                                                                       *
 *                                      3.  adaptor motif info -- sequences and size                                                        *
...
#define	LEN_NAME	76              /* needed to define the data structure for fastq read */
#define	LEN_SEQ		150
#define	LEN_TAG		16
...
Bool        remove_adaptor( struct node_fastq * );  /* search then remove adaptor, turn status -- success or fail */
...
Bool        is_node_same( struct node_fastq *, struct node_fastq *);
...
Bool        remove_adaptor( struct node_fastq * p_nod)  /* search then remove adaptor, turn status -- sucess or fail */
{
	...
	p_motif1_end = p_seq + strlen(p_seq) -15;
    p_motif1_start = p_motif1_end - len_motif1;        /* pointing to the position where motif1 is expected */
    
    strcpy(p_nod->read.tag, p_motif1_end);  /* moved tag seq to third line */
    *p_motif1_end = '\0';

    
	if (strcmp (p_motif1_start, MOTIF1) != 0) { num_reads_missing_motif1++; return FAIL;}  /* cannot find motif1 */
