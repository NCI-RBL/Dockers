...
// search 3' adaptor sequence and remove it.
...
// maybe I need to add a feature to remove all contaminants

// maybe I need to give stat about length distribution

// 2016 Jan Find out that the first 5nt of hsa-miR-1 is exact the same as adaptor. Changed the pass-rate to 0.9 and minimal length to 10. 
...
// 2021 Dec 17th, change the length of reads to 200 and named it for Ryan

// 2023 Mar 22nd, allowing users to input minimal length; changing the name to v5
...
int main(int argc, char* argv[])		
{
	if (argc != 2) 
	{
		printf("\n adaptor_remove_illumina_v5.exe min_length_pass_filter  < input.fastq > output.fastq\n"); 
		return 0;
	}

	int         min_length_pass_filter = atoi (argv[1]); // new code added 2023 march
