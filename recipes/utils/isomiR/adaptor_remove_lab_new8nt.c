...
// 2021-July-28:    v7
// for some reason unknown, the adaptor sequence is not CTGTTAAC, corrected accordingly.

// 2023-Mar-22: 1) allowing users to input minimal length (no need, they already are)
//		2) change read length to 200nt
//		3) change the name to adaptor_remove_lab_new8nt.c
...
#define	LEN_NAME	200              /* need to define the data structure for fastq read */
#define	LEN_SEQ		200
#define	LEN_TAG		16
...
#define TAG_PASS_RATE                   0.8         // allowing up to 2 mismatches in the 15N tag..
...
int main(int argc, char* argv[])		/* One argument allowed */
{
	if (argc != 2)
	{
		fprintf(stderr, "\n trim_read1_v7.exe min_length_pass_filter < input.fastq > output.fastq \n");
		return 0;
	}
    ...
    int         num_total_reads = 0, num_contain_adaptor = 0, num_empty = 0, num_too_short = 0, num_5adapotr_contamination = 0, num_final = 0, num_duplicated = 0;
